...
static struct gpio_desc *dac_obtain_pin(struct device *dev, int pin, char *name,
					int init_val)
{
...
static int __init dac_probe(void)
{
	...
	dac_dat->dac_dev = device_create(dac_dat->dac_class, NULL,
					 MKDEV(dac_dat->dac_major, 0),
					 (void *)dac_dat, "dac");
	...
	if (dac_dat->gpio_dac_b0 == NULL)
		goto fail;
	dac_dat->gpio_dac_b1 =
		dac_obtain_pin(dac_dat->dac_dev, 17, "DAC_b1", 0);
	if (dac_dat->gpio_dac_b1 == NULL)
		goto fail;
	dac_dat->gpio_dac_b2 =
		dac_obtain_pin(dac_dat->dac_dev, 18, "DAC_b2", 0);
	if (dac_dat->gpio_dac_b2 == NULL)
		goto fail;
	dac_dat->gpio_dac_b3 =
		dac_obtain_pin(dac_dat->dac_dev, 27, "DAC_b3", 0);
	if (dac_dat->gpio_dac_b3 == NULL)
		goto fail;
	dac_dat->gpio_dac_b4 =
		dac_obtain_pin(dac_dat->dac_dev, 22, "DAC_b4", 0);
	if (dac_dat->gpio_dac_b4 == NULL)
		goto fail;
	dac_dat->gpio_dac_b5 =
		dac_obtain_pin(dac_dat->dac_dev, 23, "DAC_b5", 0);
	if (dac_dat->gpio_dac_b5 == NULL)
		goto fail;
	dac_dat->gpio_dac_b6 =
		dac_obtain_pin(dac_dat->dac_dev, 24, "DAC_b6", 0);
	if (dac_dat->gpio_dac_b6 == NULL)
		goto fail;
	dac_dat->gpio_dac_b7 =
		dac_obtain_pin(dac_dat->dac_dev, 25, "DAC_b7", 0);
	if (dac_dat->gpio_dac_b7 == NULL)
		goto fail;
...
fail:
	if (dac_dat->gpio_dac_b7)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b7));
	if (dac_dat->gpio_dac_b6)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b6));
	if (dac_dat->gpio_dac_b5)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b5));
	if (dac_dat->gpio_dac_b4)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b4));
	if (dac_dat->gpio_dac_b3)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b3));
	if (dac_dat->gpio_dac_b2)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b2));
	if (dac_dat->gpio_dac_b1)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b1));
	if (dac_dat->gpio_dac_b0)
		devm_gpio_free(dac_dat->dac_dev,
			       desc_to_gpio(dac_dat->gpio_dac_b0));

	// Device cleanup
	if (dac_dat->dac_dev)
		device_destroy(dac_dat->dac_class,
			       MKDEV(dac_dat->dac_major, 0));
	// Class cleanup
	if (dac_dat->dac_class)
